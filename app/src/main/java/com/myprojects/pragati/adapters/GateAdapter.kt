...
class GateAdapter(private val items: List<Websites>) : RecyclerView.Adapter<GateAdapter.GateViewHolder>() {

    class GateViewHolder(val binding: RecyclerviewSingleItemBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): GateViewHolder {
        val binding_ = RecyclerviewSingleItemBinding.inflate(
            LayoutInflater.from(parent.context),
            parent,
            false
        )
        return GateViewHolder(binding_)
    }
                        ...
                        val docRef = favoritesRef
                            .add(
                                mapOf(
                                    ...
                                    "filter" to "GATE" // add the new field "filter" here
                                )
                            )
                            .await()
