...
class DataStructureFragment : Fragment() {

    private var _binding: FragmentDataStructureBinding? = null
    private val binding get() = _binding!!
    private lateinit var adapter: DataStructureAdapter
    ...
    override fun onCreateView(
        ...
        _binding = FragmentDataStructureBinding.inflate(inflater, container, false)

        (activity as AppCompatActivity).setSupportActionBar(binding.dataStructureToolbar)
        (activity as AppCompatActivity).supportActionBar!!.setDisplayHomeAsUpEnabled(true)
//        (activity as AppCompatActivity).supportActionBar!!.title = "Data Structure"

        // Remove the app name from the Toolbar
        (activity as AppCompatActivity).supportActionBar!!.setDisplayShowTitleEnabled(false)

        binding.dataStructureToolbar.navigationIcon?.setTintList(null) // Set the toolbar navigation icon to use the default color

        setHasOptionsMenu(true)

        adapter = DataStructureAdapter(dataLists) // Initialize RecyclerView adapter

        binding.dataStructureRecyclerView.adapter = adapter  // Set RecyclerView adapter

        binding.dataStructureRecyclerView.layoutManager = LinearLayoutManager(context)  // Set RecyclerView layout manager
    ...
    private fun fetchData() {
        ...
        val documentRef = db.collection("websites").document("datastructure").collection("items")
        documentRef.get()
